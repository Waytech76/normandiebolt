/*
  ...
  2. Sécurité
    ...
    - Politique permettant aux utilisateurs publics d'insérer des avis
...
-- Créer les politiques RLS pour public (anon + authenticated)
...
DROP POLICY IF EXISTS "allow_public_upload_review_photos" ON storage.objects;
DROP POLICY IF EXISTS "allow_public_read_review_photos" ON storage.objects;
DROP POLICY IF EXISTS "allow_authenticated_manage_review_photos" ON storage.objects;
